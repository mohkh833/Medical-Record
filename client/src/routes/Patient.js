...
class Patient extends Component {
    ...
    async componentDidMount() {
        ...
        this.setState({
            PatientInstance: instancePatient,
            account: accounts[0]
    ...
    onAddressPermissionChange = (e) => {
        this.setState({
            addressPermission: e.target.value
        })
    }
    ...
    getPatientDetails = async () => {
        ...
        await this.state.PatientInstance.methods.getPatientDetails(this.state.account).call({from: this.state.account})
        .then(result => {
            this.setState({name: result._name,
                phone:result._phone,
                gender: result._gender,
                dob:result._dob,
                bloodGroup:result._bloodGroup,
                allergy:result._allergies,
                patientView: true,
                load:false
            })
        })
    ...
    getPatientRecords = async () => {
        ...
        const result = await this.state.PatientInstance.methods
        .getPatientRecords(this.state.account).call({from:this.state.account})
        ...
        this.setState({records:recs, load:false, addRecordView:true})
    }
    ...
    onRecordSubmit = async () => {
        ...
        this.setState({load: false, addRecordView: true})
    }
                ...
                <Grid>
                    <Grid item>
                        <Paper>
                            ...
                            <Box>
                            <TextField label="Admitted On" type="date" onChange={this.onAdmittedOnChange} InputLabelProps={{shrink: true}}></TextField>
                            </Box>

                            <Box>
                            <TextField label="Disharged On" type="date" onChange={this.onDischargedOnChange} InputLabelProps={{shrink: true}}></TextField>
                            </Box>
          ...
          <div>
            ...
            <Box m={3}>
                ...
                <Grid item>
                    ...
                    this.setState({load: false, patientView: false, recordView: true, giveGrantView: false, revokeGrantView: false,  addRecordView: false})
                    }}></Chip>
                </Grid>
                <Grid item>
